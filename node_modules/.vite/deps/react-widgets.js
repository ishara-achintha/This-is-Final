...
var dates = Object.assign({}, date_arithmetic_exports, {
  merge(date2, time, defaultDate) {
    if (isNullOrInvalid(time) && isNullOrInvalid(date2) == null) return null;
    if (isNullOrInvalid(time)) time = defaultDate || /* @__PURE__ */ new Date();
    if (isNullOrInvalid(date2)) date2 = defaultDate || /* @__PURE__ */ new Date();
...
function useFocusManager2(ref, props = {}, opts = {}) {
  ...
  const events = useFocusManager(Object.assign({}, opts, {
    isDisabled,
    onChange: (focused2) => {
      if (isMounted()) setFocus(focused2);
    },
    didHandle(focused2, event) {
      let handler = props[focused2 ? "onFocus" : "onBlur"];
      if (handler) handler(event);
      if (opts.didHandle && !event.isWidgetDefaultPrevented) opts.didHandle(focused2, event);
    }
  }));
...
function chunk(array3, chunkSize) {
  ...
  while (index < length) result.push(array3.slice(index, index += chunkSize));
  return result;
}
...
function getMessages(defaults = DEFAULTS) {
  ...
  Object.keys(messages).forEach((message2) => {
    ...
    if (value == null) value = messages[message2];
...
function mergeWithDefaults(date2, number, messages2, formatOverrides = {}) {
  if (!date2 && !number) throw new Error("This component requires a Localizer but none was provided");
  return {
    formatOverrides,
...
var useLocalizer = (messages2, formats) => {
  ...
  return (0, import_react13.useMemo)(() => {
    if (!messages2 && !formats) return localizer;
...
function Calendar(_ref) {
  ...
  (0, import_react22.useEffect)(() => {
    ...
    if (!dates_default.eq(inValue, dateOrNull(last2), VIEW_UNIT[currentView])) maybeSetCurrentDate(inValue);
  });
    ...
    if (direction) {
      if (ctrl) {
        ...
        if (!dates_default.eq(currentDate, nextDate2, unit)) {
          e.preventDefault();
          if (dates_default.gt(nextDate2, currentDate, currentView)) navigate("RIGHT", nextDate2);
          else if (dates_default.lt(nextDate2, currentDate, currentView)) navigate("LEFT", nextDate2);
          else maybeSetCurrentDate(nextDate2);
        }
      }
    }
    notify(onKeyDown, [e]);
  ...
  function navigate(direction, date2) {
    ...
    if (!date2) date2 = ["LEFT", "RIGHT"].indexOf(direction) !== -1 ? nextDate(direction) : currentDate;
  ...
  function maybeSetCurrentDate(date2) {
    ...
    if (date2 === currentDate || dates_default.eq(inRangeDate, dateOrNull(currentDate), VIEW_UNIT[currentView])) return;
...
var useFocusList = ({
  ...
  const list = (0, import_react24.useMemo)(() => {
    return {
      ...
      focus(el) {
        if (!el || map.has(el)) setFocusedItem(el ? map.get(el) : void 0);
      },
      last() {
        const [items] = get();
      ...
      next({
        ...
        if (nextIdx >= items.length) {
          ...
          if (behavior === "clear") return void 0;
          return focusedItem2;
        }
  ...
  (0, import_react24.useLayoutEffect)(() => {
    ...
    if (!hasItem && _focusFirstItem || hasItem && !listRef.current.contains(focusedElement)) {
      if (_focusFirstItem) list.focus(list.first());
...
function ListOption(_ref) {
  ...
  const handleSelect = (event) => {
    if (onSelect && !disabled2) onSelect(dataItem2, event);
  };
  let classes = {
...
function isWindow(node3) {
  if ("window" in node3 && node3.window === node3) return node3;
  if (isDocument(node3)) return node3.defaultView || false;
  return false;
}

// node_modules/dom-helpers/esm/contains.js
function contains(context, node3) {
  if (context.contains) return context.contains(node3);
  if (context.compareDocumentPosition) return context === node3 || !!(context.compareDocumentPosition(node3) & 16);
}
...
function offset(node3) {
  ...
  if (!docElem || !contains(docElem, node3)) return box;
  if (node3.getBoundingClientRect !== void 0) box = node3.getBoundingClientRect();
  box = {
...
var SlideDownTransition = class extends import_react38.default.Component {
  constructor(...args) {
    super(...args);
    this.nodeRef = import_react38.default.createRef();
...
var dataValue = (dataItem2, field) => {
  if (typeof field === "function") return field(dataItem2);
  if (dataItem2 == null) return dataItem2;
  if (typeof field === "string" && typeof dataItem2 === "object" && field in dataItem2) return dataItem2[field];
  return dataItem2;
};
...
function TimeInput(uncontrolledProps) {
  ...
  const [focusEvents, focused] = useFocusManager2(ref, {
    ...
    didHandle: (focused2, e) => {
      ...
      if (!focused2) return;
      if (!e.target.dataset.focusable) (_hourRef$current = hourRef.current) == null ? void 0 : _hourRef$current.focus();
  ...
  const increment = (part, inc) => {
    ...
    if (part === "meridiem") {
      ...
      if (!isValid2(String(nextPart), part, use12HourClock != null ? use12HourClock : false)) return;
    }
    notifyChange({
      [part]: nextPart
    });
  ...
  function notifyChange(updates) {
    ...
    if (use12HourClock) {
      if (hours3 === 12) hours3 = 0;
      hours3 += meridiem2 === "PM" ? 12 : 0;
    }
    nextDate.setHours(hours3);
    nextDate.setMinutes(minutes3);
    if (seconds3 != null) nextDate.setSeconds(seconds3);
    if (milliseconds3 != null) nextDate.setMilliseconds(milliseconds3);
    onChange(nextDate, {
      lastValue: value,
      timeParts
    });
...
var DatePicker = import_react50.default.forwardRef((uncontrolledProps, outerRef) => {
  ...
  const handleChange = useEventCallback((date2, str, constrain) => {
    if (readOnly || disabled2) return;
    if (constrain) date2 = inRangeValue2(date2);
    if (onChange) {
      if (date2 == null || value == null) {
        if (date2 != value) onChange(date2, str);
  ...
  function focus() {
    ...
    if (open) (_calRef$current = calRef.current) == null ? void 0 : _calRef$current.focus();
    else (_ref$current2 = ref.current) == null ? void 0 : _ref$current2.focus();
  }
...
var DropdownListInput = import_react53.default.forwardRef(function({
  ...
  const handleAutofillDetect = ({
    ...
    let autofilling2;
    if (animationName === "react-widgets-autofill-start") autofilling2 = true;
    else if (animationName === "react-widgets-autofill-cancel") autofilling2 = false;
    else return;
  ...
  const handleKeyPress = (e) => {
    ...
    nextSearchChar(String.fromCharCode(e.which), (word) => {
      ...
      while (idx < len) {
        ...
        if (isValid3(list.toDataItem(item))) break;
        idx++;
      }
      if (idx === len) return;
...
function closest(node3, selector, stopAt) {
  if (node3.closest && !stopAt) node3.closest(selector);
  var nextNode = node3;
  do {
...
function MultiselectTag({
  ...
  const handleRemove = (event) => {
    if (!disabled2) onRemove(dataItem2, event);
  };
...
function activeElement(doc) {
  ...
  try {
    ...
    if (!active2 || !active2.nodeName) return null;
    return active2;
  } catch (e) {
    return doc.body;
  }
...
var _excluded28 = ["disabled", "readOnly", "placeholder", "innerRef", "min", "max", "localizer", "editing", "parse"];
function _extends29() {
...
var NumberPickerInput = class extends import_react60.default.Component {
  constructor(...args) {
    super(...args);
    this.state = {};
    this.input = (0, import_react60.createRef)();
  ...
  isSelectingAllText() {
    const node3 = canUseDOM_default && this.input.current;
    return !!node3 && activeElement() === node3 && node3.selectionStart === 0 && node3.selectionEnd === node3.value.length;
  }
  parseNumber(strVal) {
    let {
      localizer,
...
function NumberPicker(uncontrolledProps) {
  ...
  const handleKeyDown = useEventCallback((event) => {
    ...
    notify(onKeyDown, [event]);
    if (event.defaultPrevented) return;
    if (key === "End" && isFinite(max2)) handleChange(max2, event);
    else if (key === "Home" && isFinite(min2)) handleChange(min2, event);
    else if (key === "ArrowDown") {
      event.preventDefault();
      decrement(event);
    } else if (key === "ArrowUp") {
